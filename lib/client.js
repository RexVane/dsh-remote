/**
 * Browser half of dsh-remote: make the DSH web GUI usable on the
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const STYLE_ID = "dsh-remote-mobile-fit";
		...
		const DIRECTORY_RPC_CHANNEL = "/dsh-remote";

		/** Stable id for the custom directory browser's component stylesheet. */
		const PICKER_STYLE_ID = "dsh-remote-directory-picker";
...
`;
		...
		function mountDirectoryPickerStyle() {
			...
			style.textContent = `
.dsh-remote-picker-dialog.dsh-remote-picker-dialog {
...
.dsh-remote-picker-shell {
  display: contents;
  color: var(--dsw-alias-label-primary);
}
.dsh-remote-picker-header {
  flex: none;
  padding: 16px 18px 10px;
  border-bottom: 1px solid var(--dsw-alias-border-l3);
}
.dsh-remote-picker-title {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 510;
  line-height: 24px;
}
.dsh-remote-picker-path-row {
  display: flex;
  align-items: center;
  gap: 6px;
  min-width: 0;
}
.dsh-remote-picker-computer,
.dsh-remote-picker-path-button {
...
.dsh-remote-picker-computer:hover,
.dsh-remote-picker-path-button:hover {
  color: var(--dsw-alias-label-primary);
  background: var(--dsw-alias-interactive-bg-hover);
}
.dsh-remote-picker-chevron {
  color: var(--dsw-alias-label-tertiary);
  flex: none;
}
.dsh-remote-picker-path-input {
  box-sizing: border-box;
  min-width: 0;
  width: 100%;
  height: 30px;
...
.dsh-remote-picker-body {
  flex: 1 1 0;
  min-height: 0;
  overflow-y: auto;
  padding: 14px 14px 18px;
}
.dsh-remote-picker-drive-grid {
  display: grid;
  grid-template-columns: 1fr;
  gap: 8px;
}
.dsh-remote-picker-drive,
.dsh-remote-picker-folder {
  box-sizing: border-box;
  width: 100%;
  min-width: 0;
  color: var(--dsw-alias-label-primary);
...
.dsh-remote-picker-drive:hover,
.dsh-remote-picker-folder:hover,
.dsh-remote-picker-folder[data-selected="true"] {
  background: var(--dsw-alias-interactive-bg-hover);
}
.dsh-remote-picker-drive-mark {
  flex: none;
  width: 30px;
  height: 30px;
  border: 1px solid var(--dsw-alias-border-l2);
...
.dsh-remote-picker-folder-icon {
  flex: none;
  color: var(--dsw-alias-label-secondary);
}
.dsh-remote-picker-name {
  min-width: 0;
  flex: 1 1 0;
  overflow: hidden;
...
.dsh-remote-picker-status,
.dsh-remote-picker-truncated,
.dsh-remote-picker-error,
.dsh-remote-picker-empty {
  padding: 8px 4px;
  font-size: 12px;
  line-height: 18px;
}
.dsh-remote-picker-status,
.dsh-remote-picker-truncated,
.dsh-remote-picker-empty { color: var(--dsw-alias-label-secondary); }
.dsh-remote-picker-error { color: var(--dsw-alias-state-error-primary); }
.dsh-remote-picker-footer {
  flex: none;
  display: flex;
  flex-wrap: wrap;
  align-items: center;
...
.dsh-remote-picker-footer-gap { flex: 1 1 0; }
.dsh-remote-picker-action { min-width: 72px; }
@media (max-width: ${BREAKPOINT}) {
  .dsh-remote-picker-dialog.dsh-remote-picker-dialog {
    width: calc(100vw - 12px);
    height: min(620px, calc(100dvh - 12px));
  }
  .dsh-remote-picker-header { padding: 14px 12px 9px; }
  .dsh-remote-picker-body { padding: 10px 8px 14px; }
  .dsh-remote-picker-footer {
    flex-wrap: wrap;
  ...
  .dsh-remote-picker-footer-gap { display: none; }
  .dsh-remote-picker-action { min-width: 64px; }
}
`;
			document.head.appendChild(style);
			return () => style.remove();
		...
		function DriveDirectoryFlow(props) {
			...
			react.useEffect(() => {
				...
				props.listDrives(ac.signal).then((next) => {
					if (seq !== generation.current) return;
					const roots = Array.isArray(next) ? next.filter((path) => typeof path === "string" && (path.startsWith("/") || /^[A-Za-z]:\\$/u.test(path))) : [];
					if (roots.length === 0) {
						loadHome();
						return;
					}
			...
			return h(primitives.Modal, {
				...
				title: labels.title,
				className: "dsh-remote-picker-dialog",
				headless: true,
			}, h("div", { className: "dsh-remote-picker-shell" },
				h("div", { className: "dsh-remote-picker-header" },
					h("h2", { className: "dsh-remote-picker-title" }, labels.title),
					pathDraft !== null ? h("input", {
						className: "dsh-remote-picker-path-input",
						value: pathDraft,
					...
					}) : h("div", { className: "dsh-remote-picker-path-row", role: "navigation" },
						h("button", {
							type: "button",
							className: "dsh-remote-picker-computer",
							disabled: parentBusy,
							onClick: () => {
						...
						...pathCrumbs.map((crumb, index) => h(react.Fragment, { key: crumb.path },
							index === 0 && h(primitives.IconChevronRightOutline14, { className: "dsh-remote-picker-chevron", size: 12 }),
							h("button", {
								type: "button",
								className: "dsh-remote-picker-path-button",
								disabled: loading || parentBusy,
				...
				h("div", { className: "dsh-remote-picker-body" },
					loading && h("div", { className: "dsh-remote-picker-status", role: "status" }, labels.loading),
					!loading && computer && drives !== null && h("div", { className: "dsh-remote-picker-drive-grid", role: "list" },
						drives.map((drive) => h("button", {
							type: "button",
							className: "dsh-remote-picker-drive",
							key: drive,
							disabled: parentBusy,
							onClick: () => loadDirectory(drive),
							role: "listitem",
							}, h("span", { className: "dsh-remote-picker-drive-mark", "aria-hidden": true }, drive.startsWith("/") ? "/" : drive.slice(0, 2)), h("span", { className: "dsh-remote-picker-name" }, drive)))),
					!loading && computer && drives?.length === 0 && h("div", { className: "dsh-remote-picker-empty" }, labels.empty),
					!loading && !computer && entries.length === 0 && h("div", { className: "dsh-remote-picker-empty" }, labels.emptyFolder),
					!loading && !computer && entries.map((entry) => h("button", {
						type: "button",
						className: "dsh-remote-picker-folder",
						key: entry.path,
						disabled: parentBusy,
						onClick: () => loadDirectory(entry.path),
						}, h(primitives.IconFolderClose16, { className: "dsh-remote-picker-folder-icon", size: 16 }), h("span", { className: "dsh-remote-picker-name" }, entry.name), h(primitives.IconChevronRightOutline14, { size: 12 }))),
					!loading && !computer && listing?.truncated === true && h("div", { className: "dsh-remote-picker-truncated", role: "note" }, labels.truncated),
					error !== null && h("div", { className: "dsh-remote-picker-error", role: "alert" }, error),
				),
				h("div", { className: "dsh-remote-picker-footer" },
					h(primitives.Button, {
						variant: "outline",
					...
					h("span", { className: "dsh-remote-picker-footer-gap" }),
					h(primitives.Button, { variant: "outline", className: "dsh-remote-picker-action", disabled: parentBusy, onClick: props.onCancel }, labels.cancel),
					h(primitives.Button, { variant: "primary", className: "dsh-remote-picker-action", disabled: parentBusy || computer || targetPath === null || loading, onClick: () => !parentBusy && !loading && !computer && targetPath !== null && props.onPicked(targetPath) }, labels.open),
				),
				newFolder !== null && h(primitives.Modal, {
					...
					error !== null && h("div", { className: "dsh-remote-picker-error", role: "alert", style: { margin: 0 } }, error),
		 ...
		 */
			 ...
			 * `/dsh-remote/<endpoint>`, which the host serves with DSH's own
			...
			function installSettingsProxy(ctx) {
				...
				const original = window.__dshRemoteFetchOriginal ?? window.fetch.bind(window);
				window.__dshRemoteFetchOriginal = original;
				...
				return () => {
					...
					window.fetch = window.__dshRemoteFetchOriginal;
					delete window.__dshRemoteFetchOriginal;
				};
			}
			...
			function registerDirectoryPicker(ctx) {
				...
				ctx.effect(() => mountDirectoryPickerStyle(), "dsh-remote: directory picker stylesheet");
				ctx.effect(() => installSettingsProxy(ctx), "dsh-remote: settings plane proxy");
		...
		function apply(ctx) {
			...
			if (typeof ctx?.effect === "function") {
				ctx.effect(() => teardown, "dsh-remote: mobile fit stylesheet");
			}
		}
