 ...
 * neighbours. Nothing here is phone-specific beyond the width, so it is a
 * stylesheet, not a component: DSH's own components, state and buttons stay
 * in charge.
 *
 * Everything is inside one `@media (max-width: 820px)` block, so a desktop
 * browser is bit-for-bit unaffected.
 *
 ...
 *
 * Out of scope, on purpose: on the phone the Settings page's agent-preset and
 * permission rows still report `HTTP 403`. No stylesheet can change that:
 * `settings.describe`, credentials mutations and `llm.discoverModels` are in
 * DSH's PRIVILEGED_METHODS set, whose gate uses
 * `isTrustedApiRequest(request, [])` with a hardcoded empty trusted-host list.
 * The bundle patch separately pins DSH's browse directory picker, so adding a
 * workspace uses `host.listDirectory`, `host.createDirectory` and
 * `workspace.create` instead of privileged `host.pickDirectory`. Chat,
 * trajectory and the already-configured model catalog remain browser-safe.
 */
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Generic Connection RPC used only to enumerate ready Windows drive roots. */
		const DRIVE_RPC_CHANNEL = "/tailscale-serve";
		const DRIVE_RPC_ENDPOINT = "listDrives";
...
@media (max-width: ${BREAKPOINT}) {
  ...
  .dsh-ts-picker-footer { padding: 10px 10px calc(10px + env(safe-area-inset-bottom)); }
}
`;
			document.head.appendChild(style);
			return () => style.remove();
		...
		function DriveDirectoryFlow(props) {
			...
			return h(primitives.Modal, {
				...
				h("div", { className: "dsh-ts-picker-body" },
					...
					!loading && !computer && entries.length === 0 && h("div", { className: "dsh-ts-picker-empty" }, labels.empty),
				...
				newFolder !== null && h(primitives.Modal, {
					...
					h("input", {
						...
						onKeyDown: (event) => event.key === "Enter" && confirmFolder(),
					}),
		 ...
		 */
			...
			async function listDriveRoots(connection, signal) {
				const result = await connection.rpc.call(DRIVE_RPC_CHANNEL, DRIVE_RPC_ENDPOINT, {}, signal);
				if (!result.ok) throw new Error(result.error.message);
				const roots = result.value?.drives;
				if (!Array.isArray(roots)) throw new Error("drive list response is invalid");
				return roots;
			}

			/** Occupy the two workspace directory-flow holes with the drive-aware UI. */
			function registerDirectoryPicker(ctx) {
				if (react === null || primitives === null || ctx?.slots === undefined || ctx?.workspaces === undefined || ctx?.connection === undefined) return;
		...
		/**
		 * Client plugin body. The stylesheet needs no services — it is correct
		 * whenever it lands and for whatever DSH renders later — and the marquee
		 * watcher only reads geometry.
		 *
		 * `mount` and `watchMarquee` are unconditional: the sheet gates itself with
		 * its own @media block, and `markMarquee` re-checks the breakpoint on every
		 * pass, so both already behave correctly at any width. The other three
		 * change state the media query cannot reach — a meta attribute and two
		 * document-level capture listeners — so they are bound through `whilePhone`,
		 * which installs and removes them as the breakpoint is crossed.
		 *
		 * Opening the page with `?nomobilefit=1` skips all of it, which is the
		 * only reliable way to tell a layout fault in this sheet apart from one in
		 * DSH itself: load the same URL with and without the flag and compare.
