...
function installStaticEnhancer(ctx, deps = {}) {
	...
	function store(key, entry) {
		...
		if (previous) cacheBytes -= previous.raw.length + (previous.gzip?.length ?? 0);
		cache.set(key, entry);
		cacheBytes += entry.raw.length + (entry.gzip?.length ?? 0);
		while (cacheBytes > STATIC_ENHANCER_CACHE_BYTES && cache.size > 0) {
			const oldest = cache.keys().next().value;
			const evicted = cache.get(oldest);
			cacheBytes -= evicted.raw.length + (evicted.gzip?.length ?? 0);
			cache.delete(oldest);
		}
	}
	...
	function respond(res, req, entry) {
		const useGzip = /\bgzip\b/i.test(String(req.headers["accept-encoding"] ?? "")) && entry.gzip !== undefined;
...
function applyWithDeps(ctx, config = {}, deps = {}) {
	...
	if (!enabled) {
		// No phone surface at all: not even the /tailscale-serve channel exists,
	...
	installStaticEnhancer(ctx, deps);
