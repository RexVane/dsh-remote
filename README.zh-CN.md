...
一个 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)(DSH)插件:把 DSH 网页 GUI 通过 **Tailscale tailnet** 暴露给手机——同一个网址在家(Wi-Fi 直连)在外(DERP 中继)都能用,TLS 自动配置,无需 `--trusted-host`。
...
5. **运行 DSH 的账户有权管理 Tailscale Serve。** Windows 下,在与 `dsh web` 相同的 PowerShell 会话/账户里执行:
   ...
   输出 `{}` 即成功,只表示该节点还没有 Serve 路由,插件可以创建第一条。若返回 `Access is denied`,请用管理员 shell 运行 DSH,或按所装 Tailscale 版本支持的方式授予该账户 Serve/operator 权限。`tailscale status` 成功或能看到 `*.ts.net` 域名只证明节点在线,不证明该账户能创建/读取/条件更新 Serve 配置。
6. **Node.js 22.19+(主版本 22 内)或 24+。** DSH rc.8 排除 Node 23 与更早的 22.x;当前实际使用的是 Node 24。

## 安装

### 1. 安装 DSH(如果还没有)

插件依赖 DSH 本体。**全局安装**最可靠(npx 一次性缓存已被证明脆弱——可能损坏并拒绝重装,报 `ECOMPROMISED`):
...
`dsh plugin` 转发给 pnpm,把包装进 `web` profile,并调和进 `dsh.profile.bundles` 层叠(`package.json` 里的 `dsh.bundle.patch` 声明使它成为组合层)。

> **Windows 路径怪癖(已验证)**:pnpm 10 在 profile 目录里会错误解析盘符开头的 `file:`/`link:` 规范。如果 `dsh plugin` 报 `ENOENT ... scandir '<profile>\D:\...'`,改用手动安装:
>
> ```powershell
...
> ```
>
> 然后把 `"dsh-tailscale-serve"` 追加到 `package.json` 的 `dsh.profile.bundles` 数组(排在 `@deepseek-ai/dsh-web-app` 之后)。
...
两行日志证明的是不同的事。`added ... trust fence` 表示普通浏览器 API 流量不会再仅因 `*.ts.net` 主机名被拒。`DSH web is now reachable` 只在 Serve 命令成功退出**且** Tailscale 节点级配置里出现完全符合预期的代理路由后才打印。仅凭已知的 tailnet DNS 名从不当作成功;瞬态启动失败会重试,而权限/冲突/路由验证错误会停下,不给出虚假的成功信息。

## 手机使用

1. 手机打开 Tailscale App,确认设备**在线**(绿色)。
2. 手机浏览器打开 `https://<machine>.<tailnet>.ts.net`(例如 `https://my-pc.example-tailnet.ts.net`)。
3. 聊天、工具调用、交付物**实时**流式呈现——和电脑上同一个会话。在手机的非回环 Tailscale 页面上,新建工作区会打开插件的虚拟**此电脑**视图,列出就绪的 Windows 盘符(如 `C:\`、`D:\`);选盘符后浏览真实主机目录,可新建并选择工作区。电脑的 `http://127.0.0.1:<port>` 页面保持 DSH 原生的系统目录对话框。
4. 同一网址在外网也能用:没有直连路径时 Tailscale 走 DERP 中继。

> 手机**不需要**运行 DSH 或任何插件——只需要 Tailscale 成员身份。
...
插件通过补丁层(`cordis.patch.yml`)配置,可从 profile 的 `--patch` 覆盖:

| 键 | 默认 | 含义 |
|---|---|---|
| `enabled` | `true` | 总开关。`false` 时 DSH 只留回环——没有 Serve 路由、没有信任围栏条目、连插件的 `/tailscale-serve` RPC 通道都没有。 |
| `https` | `true` | 使用 Tailscale HTTPS(443 端口)。`false` 明确请求 80 端口纯 HTTP;仅在受信任的 tailnet 上、且确实不想要 TLS 时使用。 |
| `hostname` | *(自动探测)* | 可选的 `<machine>.<tailnet>.ts.net` URL 提示,在 Serve 路由可读之前使用。通常省略;验证过的路由的实际主机名总是获胜,过期/不匹配的提示会产生警告。 |
| `keepOnExit` | `false` | DSH 退出后保留插件施加的根 handler。默认情况下,正常关机只移除本进程证明是自己添加的路由;不会恢复或替换 Serve 的其余部分。 |
| `serveArgs` | *(省略)* | 高级覆盖:完整参数向量,如 `["serve", "--bg", "--yes", "--https=443", "http://127.0.0.1:3080"]`。必须显式配置恰好一个匹配的 HTTP(S) 监听、后台模式、并以真实 DSH 目标结尾。Service/path/TCP 转发/TUN 模式会被拒绝,因为插件无法安全地拥有/清理它们。 |
...
- **静态传输增强**:插件包装 webServer 的请求监听器,给 `/assets` 和 `/plugins` 的 GET 响应加上 gzip 压缩、ETag 和缓存头——带指纹的名字(含每个 `?rev=` 插件 bundle URL)是 `immutable`,其余用 304 重验证。DSH 原本都没有,DERP 中继下手机每次打开页面都要重新下载约 4.4MB 启动负载;增强后首次加载约 1.1MB,重复加载接近零。增强按请求的 Host 头门控:只有非回环(tailnet)请求享受,电脑的 `127.0.0.1` 浏览器仍收到 DSH 字节级一致的原生响应。响应体逐字节相同,`/api` 与事件流原样通过,且增强只在插件启用时运行。
...
如果是手动安装(`link:`),改为在 `C:\Users\<you>\.dsh\profiles\web\package.json` 里删除依赖和 `dsh.profile.bundles` 中的 `dsh-tailscale-serve` 条目,然后在该目录 `pnpm install`。
